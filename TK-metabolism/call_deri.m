...
if useode == 1 % use the ODE solver to calculate the solution
    ...
    reltol = 1e-4;
    abstol = 1e-7; %20260723 JR: Changed from 1e-7 to 1e-6 to reduce runtime but no effect
    ...
    %options =
    %odeset(options,'InitialStep',max(t)/100,'MaxStep',max(t)/100); %specify smaller stepsize %20260723 JR tested activated to reduce runtime but no effect
