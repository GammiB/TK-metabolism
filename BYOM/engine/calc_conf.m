...
disp('Calculating confidence intervals on model curves (parallel). Please wait.')
...
pmat_base = pmat; % keep original untouched; each worker gets its own copy
% Note: pmat itself is deliberately left unmodified here (unlike the old
% sequential loop, which left it mutated with the last sample's values).
% Only column 1 (values) was ever changed inside the loop; column 2 (fit
% flags), which is the only column read later in this file (see the
% sensitivity-analysis section), is untouched either way.

% pre-allocate per-sample temporary collectors (required for parfor: it
% cannot classify direct nested-indexed writes like X2{i}(:,j,k) inside
% a parfor loop, so each worker collects its own sample here instead)
Xtmp = cell(n_samples,1);
if ~isempty(namesz)
    ztmp = cell(n_samples,1);
end

glo2_local = glo2; % plain (non-global) copy so parfor can broadcast it to workers
% Note: 'global glo2' at the top of this file does NOT get sent to parallel
% workers automatically -- each worker has its own, separate, empty global
% namespace. packunpack.m relies on 'global glo2' internally. A 'global'
% statement is not allowed to appear directly inside a parfor loop body,
% so instead we push glo2 onto every worker's global namespace here, once,
% before the parfor loop starts, using a small helper function
% (set_glo2_worker.m) that is allowed to contain 'global' since it's an
% ordinary function rather than parfor-loop code.
pool_obj = gcp; % ensure a pool is running, and get a handle to it
wait(parfevalOnAll(pool_obj,@set_glo2_worker,0,glo2_local)); % set glo2 on every worker, and wait until done

parfor k = 1:n_samples % run through all samples (now in parallel)

    pmat_k = pmat_base; % local, fully-defined copy (required for parfor)
    pmat_k(ind_fit,1) = rnd(k,:); % replace values in pmat with the k-th random sample from the MCMC
    ...
    if sum(ind_logfit)>0
        pmat_k(ind_logfit,1) = 10.^(pmat_k(ind_logfit,1));
    end
    % Note: pmat_k is still on normal scale here, but the sample in rnd
    % contains the value on a log scale, if a parameter is fitted on log
    % scale. Call_deri needs normal scale structure par_k.
    pmat_k(loc_zero,1) = 0; % make parameter zero in each set of the sample!
    % this has to be done after the tranformation to normal scale!
    par_k = packunpack(2,0,pmat_k); % transform parameter matrix into a structure

    Xk = cell(n_X,n_s); % local per-sample collector for this worker
    zk = nan(1,n_zvd);  % always initialise (parfor requires unconditional assignment)
    ...
    for j = 1:n_s % run through our scenarios
        ...
        for i = 1:n_X  % run through state variables
            Xk{i,j} = Xout(:,i); % collect state variable i locally
        end
        if ~isempty(namesz)
            for i_zvd = 1:n_zvd
                zk(i_zvd) = zvd.(namesz{i_zvd})(3); % Note: this collects only LAST scenario!
            end
        end
    end
...
% Reassemble results into X2/zvd_coll (fast: no ODE solving here, kept serial)
for k = 1:n_samples
    for j = 1:n_s
        for i = 1:n_X
            X2{i}(:,j,k) = Xtmp{k}{i,j};
